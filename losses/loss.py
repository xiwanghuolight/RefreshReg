...
class Loss(nn.Module):
    ...
    def forward(self, coords_src, coords_tgt, feats_src, feats_tgt, feats_src_m, feats_tgt_m, feats_src_l, feats_tgt_l, coors, transf, w_saliency):
        ...
        circle_loss_h = self.circle_loss(coords_dist=coords_dist,
                                         feats_dist=feats_dist)
        circle_loss_m = self.circle_loss(coords_dist=coords_dist,
                                          feats_dist=feats_dist_m)
        circle_loss_l = self.circle_loss(coords_dist=coords_dist,
        ...
        loss_dict['circle_loss'] = circle_loss_h
        loss_dict['circle_loss_m'] = circle_loss_m
        loss_dict['circle_loss_l'] = circle_loss_l
        loss_dict['recall'] = recall
        loss_dict['recall_m'] = recall_m
        loss_dict['recall_l'] = recall_l

        loss = self.w_circle_loss * circle_loss_h + \
               self.w_circle_loss * circle_loss_m + \
               self.w_circle_loss * circle_loss_l + \
